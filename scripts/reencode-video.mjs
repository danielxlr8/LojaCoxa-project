...
const output = path.resolve(__dirname, "../public/videos/torcida_scrub.mp4");
...
const args = [
  ...
  "-c:v", "libx264",
  "-g", "1",          // GOP size = 1 → todo frame é keyframe
  "-keyint_min", "1",
  "-sc_threshold", "0",
  "-bf", "0",          // sem B-frames
  "-crf", "20",        // qualidade alta (0=lossless, 23=default, 51=pior)
  "-preset", "fast",   // equilíbrio entre velocidade de encode e tamanho
  "-profile:v", "baseline", // compatibilidade máxima com browsers
  "-level", "3.1",
  "-pix_fmt", "yuv420p",
...
execFile(ffmpegPath, args, { maxBuffer: 1024 * 1024 * 50 }, (err, stdout, stderr) => {
  ...
  console.log("\n⚡ Agora atualize o src no BrandStory.tsx para '/videos/torcida_scrub.mp4'");
});
