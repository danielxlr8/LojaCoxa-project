...
import { Search, Package, Truck, CheckCircle, Clock, User, UserMinus } from "lucide-react";
import Image from "next/image";
import Link from "next/link";

// MOCK Order Status (In a real app, fetched from API)
const MOCK_ORDER = {
  id: "CX-9821-44B",
  status: "shipped", // "processing" | "shipped" | "out_for_delivery" | "delivered"
  date: "12 de Abril, 2026",
  items: [
    { name: "Camisa I 2026 Jogador", size: "G", image: "https://cdn.fanmarket.app.br/69c863e72a35aa109ae884d4_camisa-jogo-1-mas.png" }
  ]
};
...
export default function TrackOrderPage() {
  ...
  const [orderResult, setOrderResult] = useState<typeof MOCK_ORDER | null>(null);
  
  // Track the user flow stage
  const [viewState, setViewState] = useState<"selection" | "guest_form" | "results">("selection");
  ...
  // Timeline Steps
  const steps = [
    { id: "processing", icon: Clock, title: "Pedido Confirmado", desc: "Seu pagamento foi aprovado" },
    { id: "shipped", icon: Package, title: "Em Separação e Envio", desc: "Saindo do nosso centro de distribuição" },
    { id: "out_for_delivery", icon: Truck, title: "Em Rota", desc: "Chegando no seu endereço hoje" },
    { id: "delivered", icon: CheckCircle, title: "Entregue", desc: "Aproveite seu manto coxa-branca!" }
  ];

  const getCurrentStepIndex = () => {
    if (!orderResult) return -1;
    const orderStatuses = ["processing", "shipped", "out_for_delivery", "delivered"];
        ...
        <div className="w-full max-w-[600px]">
          ...
          <div className="text-center mb-10">
            <h1 className="text-4xl md:text-5xl font-black font-heading uppercase tracking-tight text-white mb-2">
              Acompanhe seu <br/> <span className="text-[var(--color-primary)]">Pedido</span>
            </h1>
            ...
            {viewState === "selection" && (
              ...
              >
                <div className="w-full max-w-[500px] flex flex-col gap-4">
                  ...
                  >
                    Pedido com conta Coxa
                    <User size={20} className="group-hover:text-[var(--color-primary)] transition-colors"/>
                  </Link>
                  <button 
                    onClick={() => setViewState("guest_form")}
                    className="w-full bg-[#111] border border-white/20 hover:border-white transition-colors h-20 rounded-full flex items-center justify-center gap-3 text-white font-bold uppercase tracking-wider group"
                  >
                    Pedido sem conta Coxa
                    <UserMinus size={20} className="group-hover:text-[var(--color-primary)] transition-colors"/>
                  </button>
                </div>

                <div className="mt-8 flex flex-col items-center text-sm font-bold gap-3">
                  <div className="flex gap-2 text-white">
                    Deseja fazer uma devolução? 
                    <button className="text-[var(--color-primary)] hover:underline">Saiba mais.</button>
                  </div>
            ...
            {viewState === "guest_form" && (
              ...
              >
                ...
                <form onSubmit={handleSearch} className="space-y-4">
                  <FloatingInput 
                    label="Número do Pedido (Ex: CX-1234)" 
                    type="text" 
                    value={orderNumber}
                  ...
                  >
                    {isSearching ? (
                      ...
                      <>Rastrear Pacote <Search size={18} /></>
                    )}
                  </motion.button>
                </form>
              </motion.div>
            ...
            {viewState === "results" && orderResult && (
              ...
              >
                ...
                <div className="bg-[#0A0A0A] border border-white/10 p-6 rounded-[2px] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  <div>
                    <p className="text-[10px] uppercase font-bold text-white/40 tracking-widest mb-1">Status Oficial</p>
                    <h2 className="text-xl md:text-3xl font-black font-heading uppercase text-white tracking-tight">Pedido {orderResult.id}</h2>
                  </div>
                  <button 
                    onClick={() => {setOrderResult(null); setViewState("selection");}}
                         ...
                         <div key={step.id} className="flex gap-6 relative group">
                           ...
                           <motion.div 
                             ...
                             className={`relative z-10 w-12 h-12 flex-shrink-0 flex items-center justify-center rounded-full border-2 transition-colors duration-500
                               ${isActive ? "bg-[var(--color-primary)] border-[var(--color-primary)] text-black shadow-[0_0_20px_rgba(96,232,97,0.3)]" 
                               : isPast ? "bg-[#0A0A0A] border-[var(--color-primary)] text-[var(--color-primary)]" 
                               : "bg-[#0A0A0A] border-white/10 text-white/30"}`}
                           >
                             <step.icon size={20} strokeWidth={isPast ? 2.5 : 2} />
                           </motion.div>
                           ...
                           >
                             <h3 className={`font-bold uppercase tracking-wider text-sm ${isActive ? "text-[var(--color-primary)]" : "text-white"}`}>
                               {step.title}
                             </h3>
                             ...
                             {/* Miniatura do Item caso seja o estágio ativo (Extra UI Touch) */}
                             {isActive && (
                               <motion.div 
                               ...
                               >
                                 <Image src={orderResult.items[0].image} alt="Produto" width={40} height={40} className="object-cover rounded bg-black/50" />
                                 <div className="flex flex-col justify-center">
                                   <span className="text-[10px] text-white/40 uppercase tracking-widest font-bold">Processando</span>
                                   <span className="text-xs text-white font-medium truncate max-w-[180px]">{orderResult.items[0].name}</span>
                                 </div>
                               </motion.div>
                             )}
                           </motion.div>
