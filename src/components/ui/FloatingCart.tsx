...
export function FloatingCart() {
  const { cartItems, toggleCart, isCartOpen } = useStore();
      ...
      {isVisible && !isCartOpen && totalItems > 0 && (
        ...
        >
          <div className="relative">
            ...
            <span className="absolute -top-3 -right-3 bg-black text-white text-[11px] font-bold h-5 w-5 rounded-full flex items-center justify-center border-2 border-[var(--color-primary)]">
              {totalItems}
            </span>
          </div>
        </motion.button>
