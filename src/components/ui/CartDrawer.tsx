...
export function CartDrawer() {
  const { cartItems, isCartOpen, setCartOpen, removeFromCart, updateQuantity } = useStore();
            ...
            {cartItems.length > 0 && (
              <div className="p-6 md:p-8 bg-[#050505] border-t border-white/5">
                ...
                <Link 
                  href="/checkout"
                  onClick={() => setCartOpen(false)}
                  className="block w-full text-center bg-white text-black font-black font-heading uppercase text-xl py-5 rounded-[2px] transition-transform active:scale-[0.98] hover:bg-[#60E861] duration-300 relative overflow-hidden group"
                >
                  <span className="relative z-10 block transition-transform duration-500 ease-[var(--ease-premium)] group-hover:-translate-y-full">Finalizar Compra</span>
                  <span className="absolute inset-0 z-10 flex items-center justify-center transition-transform duration-500 ease-[var(--ease-premium)] translate-y-full group-hover:translate-y-0 text-black">Avançar →</span>
                </Link>
              </div>
            )}
          </motion.div>
